...
def CreateLog(FolderName):
    ...
    # Running Process
    Data2 = RunningProcess()

    for info in Data2:
        fobj.write("Name : %s\n" %info.get("name"))
        fobj.write("PID : %s\n" %info.get("pid"))
        fobj.write("Created Threads : %s\n" %info.get("createdthreads"))
        fobj.write(Border+"\n")
...
def ProcessScan():
    ...
    for proc in psutil.process_iter():
        try:
            ...
            try:
                info["ofd"] = proc.num_fds()
            except psutil.AccessDenied:
                info["ofd"] = "Access Denied"
    ...
    for proc in psutil.process_iter():
        try:
            if(proc.status()== "running"):
                ...
                info["createdthreads"] = proc.num_threads()
                ...
                listrunningprocess.append(info)                
        except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess):
            pass
